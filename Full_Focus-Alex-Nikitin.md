# Чек лист https://ads.vk.com:

## Кампании https://ads.vk.com/hq/dashboard

### Проверка создания кампании

* При переходе на вкладку отображается окно

![image](https://github.com/user-attachments/assets/fe58b558-ac55-4008-9cac-3fc4db1ba95d)

* При нажатии на кнопку "Создать" открывается окно создания кампании

![image](https://github.com/user-attachments/assets/0dbd741f-2eed-4655-97a7-eb35d76763aa)

![image](https://github.com/user-attachments/assets/91f8fc60-27ff-43a1-b78c-f56713990774)

* При нажатии на кнопку редактирования названия кампании в открывшемся input вводится название новой кампании

![image](https://github.com/user-attachments/assets/4407c6fb-de67-4152-92ad-4be2cd4ae119)

* При нажатии на кнопку подтвержения нового названия происходит сохнанение записи

![image](https://github.com/user-attachments/assets/06921762-53de-4890-aa0a-07bac6cc3f24)

* При нажатии на "Сообщество и профиль" открывается окно редактирования кампании

![image](https://github.com/user-attachments/assets/8cdddd8d-fdae-452e-8f36-75fd82200b30)

* При нажатии на дропдаун "Сообщество и профиль" отрывается выбор объекта рекламной кампании

![image](https://github.com/user-attachments/assets/d0da68f7-b39c-4861-91b5-38bd4762c861)

* При нажатии на кнопку "Другое сообщество" открывается модальное окно, в котором требуется указать ссылку на сообщество

![image](https://github.com/user-attachments/assets/48cb855c-34a7-4ade-8863-71c4db64bce4)

* При добавлении существующего сообщества происходит выбор сообщества как объекта рекламы

![image](https://github.com/user-attachments/assets/dca87767-c79c-4021-962f-4188af8c9149)

![image](https://github.com/user-attachments/assets/ee54d657-cd40-44f9-8c29-6b12be00f329)

* При вводе в input "Важные детали и отличия от конкурентов" и нажатии на любую часть страницы вне input данные сохраняются

![image](https://github.com/user-attachments/assets/159568bc-acbc-4ef9-bdc8-b6d89ecbf04c)

* При нажатии на input "Даты проведения" открывается календарь для ввода дат

![image](https://github.com/user-attachments/assets/6816f60f-13cb-4667-80f4-c969aefb18c1)

* При нажатии на дату в календаре она автоматически заполняет input

![image](https://github.com/user-attachments/assets/efe952d7-ced4-4dc5-8622-c0a99195af57)
![image](https://github.com/user-attachments/assets/a05c3c05-0c50-456f-8cd1-694b6d0f192f)

* При нажатии на дропдаун "Целевое действие" отображаются кнопки "Подписка на сообщество", "Отправка сообщения", "Продвижения записи".

![image](https://github.com/user-attachments/assets/ef03af25-84ac-4006-945d-0b1654f65223)

 * При нажатии на "Отправка сообщения" целевым действием выбирается именно отправка сообщения.

![image](https://github.com/user-attachments/assets/2127593f-b49a-4674-ae50-a75cd9c80c37)


* При вывключении переключателя "Оптимизация бюджета кампании" подробный список настройки скрывается

* При включении переключателя "Оптимизация бюджета кампании" отображается подробный список 


![image](https://github.com/user-attachments/assets/a16a9df6-1f02-4b90-96f8-495b1c883700)

![image](https://github.com/user-attachments/assets/91462f7a-79dc-4a1b-bd3b-d19fac5a17ce)

* При нажатии на дропдаун "Стратегия ставок" отображаются кнопки "Предельная цена", "Минимальная цена"

![image](https://github.com/user-attachments/assets/43a361cd-5479-48db-a681-db32c167e4ed)

* При нажатии на "Минимальная цена" открывается редактирование кампании в соответствии с этой стратегией ставок

![image](https://github.com/user-attachments/assets/38a767a6-2dc1-41db-8b83-21e7766dcfac)

* При нажатии на дропдаун отображаются кнопки "за день", "за все время"

![image](https://github.com/user-attachments/assets/bfd8a0ef-c712-4115-909d-ab5721289458)

* При нажатии кнопки "за все время" отображается соответсвтвующий выбор

 ![image](https://github.com/user-attachments/assets/eade6010-d51e-404f-91a0-bd48b73d8455)

* При нажатии кнопки продолжить происходит редирект на создание группы объявлений

![image](https://github.com/user-attachments/assets/a3808333-9903-456d-8841-679c8ab5d9a0)

* При нажатии на кнопку редактирования названия группы объявлений в открывшемся input вводится название новой группы объявлений

![image](https://github.com/user-attachments/assets/a77deb49-6993-4ab0-aa31-834c1529c691)

* При нажатии на кнопку подтвержения нового названия происходит сохнанение записи

![image](https://github.com/user-attachments/assets/52b7bac1-c281-4cf7-84e4-149ec220b598)

* При нажатии на кнопку "Россия" в блоке "Регионы показа" происходит выбор региона

![image](https://github.com/user-attachments/assets/b48cae49-dc64-4db3-beff-8b26d608b3fd)

![image](https://github.com/user-attachments/assets/be27400a-5f1b-437c-9ee6-45d56ef56ae3)

* При нажатии на переключатель "Расширение аудитории" аудитория расширяется

![image](https://github.com/user-attachments/assets/f5305705-3be1-4ef9-92d2-f2490ab7dcfd)

![image](https://github.com/user-attachments/assets/09275d29-fffa-43a8-ad16-31f8571adca2)

* При нажатии на дропдаун "Возрастное ограничение" отображаюся кнопки ограничений: 0+, 6+, 12+, 16+, 18+

![image](https://github.com/user-attachments/assets/1f0e08db-0186-4f57-b073-748cc48f1c4c)

* При нажатии на 12+ происходит выбор возрастной маркировки 12+

![image](https://github.com/user-attachments/assets/04173eb4-010a-4c0a-bf4d-f5367dcef3e6)

* При нажатии на "Интересы" в блоке "Интересы и поведение" открывается input

![image](https://github.com/user-attachments/assets/198ff379-5cf2-4454-8750-e25d41711562)

* При нажатии кнопки добавляется интерес "Авто внедорожники"

![image](https://github.com/user-attachments/assets/e5862af6-98cb-43e8-b851-6faa395782a7)

* При нажатии кнопки "Продолжить" происходит переход на создание объявления

![image](https://github.com/user-attachments/assets/3cb4f02f-3cf5-4111-b7b8-5c7c87bbd722)

* При вводе в input описания оно сохраняется

![image](https://github.com/user-attachments/assets/90b9aa6c-b230-4d3a-b4a1-e7565fad48b7)

* При нажатии кнопки "Созданное нейросетью" открывается медиатека

![image](https://github.com/user-attachments/assets/79f49eea-8433-4333-977b-113add203e06)

* При нажатии на любую картинку появляется кнопка "Добавить"

![image](https://github.com/user-attachments/assets/08fabc68-9aa3-4fde-b403-7059348ead3e)

* При нажатии кнопки "Добавить" изображение добавляется

![image](https://github.com/user-attachments/assets/677a5eb5-35a7-4ca2-964d-400a3de39fd5)

* При нажатии кнопки "Опубликовать" происходит редирект на страницу кампаний (после редиректа необходимо проверить наличие кампании с этим названием, с бюджетом и целевым действием на странице)

![image](https://github.com/user-attachments/assets/f1dae6dc-2143-4c93-9e3f-dcf32f7c3b38)

* При нажатии на checkbox кампании "Тест" происходит ее выбор

![image](https://github.com/user-attachments/assets/8d7143db-c96d-4c54-8b17-79dae5e1203f)

* При нажатии на дропдаун "Действия" отображаются кнопки "Остановить","Редактировать","Удалить"

![image](https://github.com/user-attachments/assets/57677c67-a2e9-4b71-8c96-95dbc3ce891d)

* При нажатии на кнопку удалить происходит удаления кампании (для консистентности)

![image](https://github.com/user-attachments/assets/f5c632b0-5916-4c7c-b071-0a9582fd7ed3)

### Проверка редактирования кампании

* При переходе на вкладку отображается окно

![image](https://github.com/user-attachments/assets/fe58b558-ac55-4008-9cac-3fc4db1ba95d)

* При нажатии на кнопку редактирования открывается модальное окно редактирования кампании

![image](https://github.com/user-attachments/assets/27d68fe3-efa3-4421-9f24-84298eb63ba0)

![image](https://github.com/user-attachments/assets/f2226088-f0ea-4683-abb7-fa8020ed78d2)

* При нахождении во вкладке редактировия кампании и нажатия на название кампании открывается редактирование название кампании

![image](https://github.com/user-attachments/assets/e5dd8d40-0cd3-4a80-9abf-3bba9f82e65c)

* При нахождении во вкладке редактировия кампании и нажатия на даты проведения кампании открывается редактирование даты проведения кампании

![image](https://github.com/user-attachments/assets/77fa035f-2394-45be-b398-0632a48e742a)

* При нахождении во вкладке редактировия кампании и нажатия на input открывается редактирование бюджета

![image](https://github.com/user-attachments/assets/435ad773-5494-4db6-b139-3e6c7ad77780)

* При нахождении во вкладке редактировия группы и нажатия на input "Регионы показа" открывается ввод региона

![image](https://github.com/user-attachments/assets/e8eeb13b-a5b3-4e74-abd2-c21258ee089e)

* При нахождении во вкладке редактировия группы и ввода в input "Регионы показа" "Санкт-Петербург" выводится кнопка города

![image](https://github.com/user-attachments/assets/0e3cc93d-8754-4be0-9a66-cfcd3ec8a8e3)

* При нахождении во вкладке редактировия группы и нажатии на кнопку "Санкт-Петербург" происходит добавление региона в список показа

![image](https://github.com/user-attachments/assets/bd284357-e8ca-4737-8b61-a4a2f493dfac)

* При нахождении во вкладке редактировия объявления и ввода в input "Описания" нового описания оно сохраняется

![image](https://github.com/user-attachments/assets/21ffc1af-4255-4ca9-b719-09b396eeb8b3)

![image](https://github.com/user-attachments/assets/d2daf4b2-c044-4aa6-af54-a5bb128a7525)

* При нажатии на кнопку "Сохранить" происходит сохранение редактирование кампании (после требуется проверить новое название, бюджет и даты проведения, а также наличие Спб в регионах показа в редактировании и нового описания)

![image](https://github.com/user-attachments/assets/6ee7596c-b544-4593-843a-125a868eef84)


## Аудитории https://ads.vk.com/hq/audience

### Проверка создания аудитории

* При нажатии на вкладку Аудитории отображается страница аудиторий

![image](https://github.com/user-attachments/assets/ea99d8c4-ca04-4a9e-bd39-5df9f6c81c9d)

* При нажатии на кнопку "Создать аудиторию" открывается модальное окно создания аудитории

![image](https://github.com/user-attachments/assets/df2fcfe2-e592-4eac-aaa7-bd19a32e025e)

* При нажатии на input "Название" открывается ввод названия новой аудитории

![image](https://github.com/user-attachments/assets/efbd0b73-d83f-4cc1-bf36-cc436824e658)

* При вводе данных и нажатии на любую часть модального окна происходит сохранение введенных данных

![image](https://github.com/user-attachments/assets/504f2ea9-ca09-4ad8-9896-15d9406be1f1)

* При нажатии на дропдаун "хотя бы одному из условий" отображаются кнопки "всем условиям", "ни одному из условий"

![image](https://github.com/user-attachments/assets/3f14c382-1d24-4a60-8437-6c199db58207)

* При нажатии на кнопку "всем условиям" происходит выбор пользователей по критерию: "соответствует всем условиям"

![image](https://github.com/user-attachments/assets/f9752487-7861-412e-a11e-559a521fc976)

* При нажатии кнопки "Добавить источник" открывается модальное окно

![image](https://github.com/user-attachments/assets/62b6acac-5fe8-4d60-a53e-40f242deb355)

* При нажатии на кнопку "Ключевые фразы" открывается окно редактирования

![image](https://github.com/user-attachments/assets/ee244f2b-1ea9-4638-9667-656b13ab6816)

* При нажатии на input "Название" открывается ввод данных

![image](https://github.com/user-attachments/assets/fc7fdd22-e259-462f-8a57-7afa7e26c01b)

* При нажатии на input "Ключевые фразы" открывается ввод данных

![image](https://github.com/user-attachments/assets/e444c79c-d427-43cc-a837-869d28882ae5)

* При нажатии на input "Минус-фразы" открывается ввод данных

![image](https://github.com/user-attachments/assets/6da030f4-1d0a-4fb4-b85b-d48a4ec83ed2)

* При нажатии на input "Период поиска" открывается ввод данных

![image](https://github.com/user-attachments/assets/73b461c2-603f-4ea2-b4b1-d6a431950682)

* При нажатии кнопку "Сохранить" происходит создание ключевых фраз и закрытие модального окна

![image](https://github.com/user-attachments/assets/932fb9e2-c31f-4197-bdd2-b539056b3f96)

* При нажатии на кнопку "Сохранить" происходит создание аудитории и закрытие модального окна (после необходимо проверить наличие аудитории с этим названием и ключевой фразой на странице)

![image](https://github.com/user-attachments/assets/f2c575be-3859-4bd3-a5c2-1f151bb15361)

* При нажатии на аудиторию высвечиваются ключевые фразы (которые нужно проверить на совпадение ввода)

![image](https://github.com/user-attachments/assets/a1e2bb73-f6b6-4fe6-ae96-2915b9f270d0)

* При нажатии на checkbox аудитории "Тест" происходит ее выбор

![image](https://github.com/user-attachments/assets/76fecefd-9447-48aa-b040-6d13442c2dd7)

* При нажатии на "Мусорное ведро" открывается модальное окно удаления

![image](https://github.com/user-attachments/assets/ba13142c-ecdc-4bc4-961d-67a97e0b7256)

* При нажатии кнопки "Удалить" происходит удаление аудитории и закрытие модального окна удаления (для консистентности)

![image](https://github.com/user-attachments/assets/52d8d9ab-06c1-432d-aae8-1e4875de8a9e)

### Проверка редактирования аудитории

* При нажатии на вкладку Аудитории отображается страница аудиторий

![image](https://github.com/user-attachments/assets/ea99d8c4-ca04-4a9e-bd39-5df9f6c81c9d)

* При нажатии на троеточие отображается кнопка редактирования аудитории

![image](https://github.com/user-attachments/assets/ceb918ca-f4e1-4801-bc27-e1f8f973ae70)

* При нажатии на кнопку редактирования аудитории открывается модальное окно редактирования

![image](https://github.com/user-attachments/assets/bd913d58-b9eb-4656-a035-5522e209e98a)

* При вводе в input "Название" сохраняется и отображается новое название аудитории

![image](https://github.com/user-attachments/assets/aa202bbc-2c4c-4432-839c-fad82005b56d)

* При нажатии на кнопку редактирования ключевых фраз открывается модальное окно редактирования

![image](https://github.com/user-attachments/assets/d169e916-28b9-42a1-87d5-2757624d2314)

* При вводе в input отображаются измененные ключевые фразы

![image](https://github.com/user-attachments/assets/c0117660-f836-41fa-9c0e-aea68938a903)

* При нажатии сохранить измененные ключевые фразы сохраняются

![image](https://github.com/user-attachments/assets/d8fa32d4-d7ac-46d9-8a2f-3a31efbf8421)

* При нажати сохранить измененная аудитория сохраняется

![image](https://github.com/user-attachments/assets/c4524c80-152a-475b-8d18-670382f50805)

* Поверить новое название и новые ключевые фразы

![image](https://github.com/user-attachments/assets/1e33c3ee-c819-4474-9955-c242840031fe)


## Обзор https://ads.vk.com/hq/overview

### Проверка избранного

* При переходе на вкладку отображается окно 

![image](https://github.com/user-attachments/assets/8dd254c5-23f1-49bb-a166-21b9d6bdd2e2)

* При нажатии на кнопку "Выбрать кампании" открывается окно с выбором кампаний

![image](https://github.com/user-attachments/assets/23afef52-01c0-409c-bf7c-8d695766e7b6)

* При нажатии на кампанию она определяется как выбранная

![image](https://github.com/user-attachments/assets/e7619de2-6964-407e-a152-eb1a15e89e3a)

* При нажатии кнопки "Cбросить" все выбранные кампании становяться невыбранными
* При нажатии "Отменить" выбор в избранное отменяется
* При нажатии на крестик выбор в избранное отменяется
* При нажатии "Сохранить" выбор сохраняется (после сохранения необходимо проверить наличие в избранном выбранной/выбранных кампаний)

![image](https://github.com/user-attachments/assets/ffaf809a-f7a9-431a-b7aa-bf5d9161996a)

![image](https://github.com/user-attachments/assets/91bbc8b2-4e29-4347-af25-22080b0ca82f)

* При нажатии на крестик на иконке кампании она удаляется из избранного (для консистентности)

![image](https://github.com/user-attachments/assets/af93ce37-ccab-434b-a625-13f2cae281ce)


### Проверка настройки дат и формата статистики

* При переходе на вкладку отображается окно 

![image](https://github.com/user-attachments/assets/8dd254c5-23f1-49bb-a166-21b9d6bdd2e2)

* При нажатии на кнопку "Выбрать кампании" открывается окно с выбором кампаний

![image](https://github.com/user-attachments/assets/23afef52-01c0-409c-bf7c-8d695766e7b6)

* При нажатии на кампанию она определяется как выбранная

![image](https://github.com/user-attachments/assets/e7619de2-6964-407e-a152-eb1a15e89e3a)

* При нажатии "Сохранить" выбор сохраняется

![image](https://github.com/user-attachments/assets/91bbc8b2-4e29-4347-af25-22080b0ca82f)

* При нажатии на календарь открывается окно выбора дат для вывода статистики

![image](https://github.com/user-attachments/assets/9e5c56a1-68b7-46c8-8967-acdcefef5c40)

* При нажатии на кнопку "Подписки" открывается модальное окно с выбором того, что будет отображено на графике статистики  

![image](https://github.com/user-attachments/assets/a05d6b28-011a-4fd2-b6ad-21fc486be388)

![image](https://github.com/user-attachments/assets/fa72a3fd-6126-45d6-9d1c-2eef52e634b0)

* При нажатии кнопки "Продажи ВКонтакте" происходит выбор данных, отображаемых на графике

![image](https://github.com/user-attachments/assets/d05e20cb-0ea2-4a20-94e7-9b29cd459007)

* При нажатии кнопки "Применить" модальное окно закрывается и происходит отображение выбранных данных (проверить что отображаются выбранные даты в параметрах статистики)

![image](https://github.com/user-attachments/assets/4ab4140b-1462-421a-8621-e5c64ec09f92)

* При нажатии на крестик на иконке кампании она удаляется из избранного (для консистентности)

![image](https://github.com/user-attachments/assets/af93ce37-ccab-434b-a625-13f2cae281ce)


### Проверка расширенной статистики

* При переходе на вкладку отображается окно 

![image](https://github.com/user-attachments/assets/8dd254c5-23f1-49bb-a166-21b9d6bdd2e2)

* При нажатии на кнопку "Выбрать кампании" открывается окно с выбором кампаний

![image](https://github.com/user-attachments/assets/23afef52-01c0-409c-bf7c-8d695766e7b6)

* При нажатии на кампанию она определяется как выбранная

![image](https://github.com/user-attachments/assets/e7619de2-6964-407e-a152-eb1a15e89e3a)

* При нажатии "Сохранить" выбор сохраняется

![image](https://github.com/user-attachments/assets/91bbc8b2-4e29-4347-af25-22080b0ca82f)

* При нажатии на название кампании открывается ее расширенная статистика

![image](https://github.com/user-attachments/assets/6f3510cb-77ba-469c-976e-e669b7e871ff)

* При нажатии на группу открывается расширенная статистика группы

![image](https://github.com/user-attachments/assets/55284224-9258-4a2d-90e7-0b71b05e096e)

* При нажатии на объявление открывается расширенная статистика объявления

![image](https://github.com/user-attachments/assets/f7acfb94-04c5-40c1-aced-9e4082d67053)

* При нажатии кнопки "скачать отчет" происходит скачивание отчета в формате xlsx (проверить наличие скаченного файла)

* При нажатии на крестик модальное окно расширенной статистики закрывается

![image](https://github.com/user-attachments/assets/8dd254c5-23f1-49bb-a166-21b9d6bdd2e2)

* При нажатии на крестик на иконке кампании она удаляется из избранного (для консистентности)

![image](https://github.com/user-attachments/assets/af93ce37-ccab-434b-a625-13f2cae281ce)


### Проверка пополнения бюджета

* При переходе на вкладку отображается окно 

![image](https://github.com/user-attachments/assets/8dd254c5-23f1-49bb-a166-21b9d6bdd2e2)

* При нажатии кнопки "Пополнить" открывается модальное окно пополнения бюджета

![image](https://github.com/user-attachments/assets/523de184-1f7e-4c85-9b2d-fbfaab84ff83)

* При вводе суммы вне диапазона от 600 до 200 000 выводится ошибка

![image](https://github.com/user-attachments/assets/aeb36261-36fb-412b-8a9a-d552b428528d)

* При нажатии на кнопку "Пополнить счет" открывается модальное окно обработки платежа (проверить его отображение)

![image](https://github.com/user-attachments/assets/28131c65-ceda-4799-9bc2-492b44db3882)

* При нажатии на любую часть вне модального окна оно закрывается

![image](https://github.com/user-attachments/assets/8dd254c5-23f1-49bb-a166-21b9d6bdd2e2)

